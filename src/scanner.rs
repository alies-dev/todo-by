...
pub fn scan_text(file_label: &str, text: &str, ctx: &ScanCtx, findings: &mut Vec<Finding>) {
    ...
    for (idx, line) in text.lines().enumerate() {
        ...
        let classified = match deadline(written) {
            None => Some((Kind::InvalidDate, None)),
            Some(due) if due <= ctx.today => Some((Kind::Overdue, Some(due))),
            Some(due) => match ctx.warn_until {
                Some(w) if due <= w => Some((Kind::DueSoon, Some(due))),
                _ => None,
            },
        };
        let Some((kind, deadline_opt)) = classified else {
            continue;
        };
...
mod tests {
    ...
    fn due_soon_within_warn_window_overdue_beyond_and_before() {
        ...
        assert_eq!(findings[0].deadline, Date::new(2999, 1, 10));
        ...
        assert_eq!(findings[0].deadline, Date::new(2998, 12, 31));
    }
