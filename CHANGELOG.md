...
- The issue spellings GitHub itself renders as links (`owner/repo#123`, `repo#123`, `GH-123`) are reported rather than ignored, each naming the accepted spelling; the cross repository form quotes the exact URL to write instead. A token carrying no `#`-with-a-digit marker stays prose, so a project matching on `todo` does not have every undated TODO reported.
